...
def fetch_papers(keyword, offset=0, limit=100):
    # sort=relevance가 아닌 publicationDate:desc로 최신성 보장
...
def main():
    ...
    print("검색을 시작합니다...")
    for kw in KEYWORDS:
        # 검색 범위를 키워드당 300개로 확대 (총 약 2,700개 후보군 훑기)
        for page in range(3): 
    ...
    with open(filename, "w", encoding="utf-8") as f:
        ...
        if not final_papers:
            f.write("> **알림:** 새로운 타겟 저널 논문이 발견되지 않았습니다. (검색 범위는 충분했으나 신규 발행물이 없을 수 있습니다.)\n\n")
        else:
